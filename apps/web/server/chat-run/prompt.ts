...
8. add_part_to_assembly — once the parts exist, combine them into the assembly. When the user asks to assemble, combine, or put existing parts together, ALWAYS use this tool: it imports the real part files, so edits to a part keep flowing through to the assembly. Never re-model an existing part into one big script to "assemble" it — that discards the original geometry and its parameters. Position parts afterwards by patching the assembly file with save_cad_script.
