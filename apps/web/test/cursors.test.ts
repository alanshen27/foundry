...
describe("CAD cursor coordinates", () => {
  it("scopes peers to the active CAD component", () => {
    expect(cadCursorSurface("part-1")).toBe("cad:part-1");
    expect(cadCursorSurface("part-1")).not.toBe(cadCursorSurface("part-2"));
  });

  it("clamps normalized viewport coordinates", () => {
    expect(normalizedCursorCoordinate(-0.2)).toBe(0);
    expect(normalizedCursorCoordinate(0.42)).toBe(0.42);
    expect(normalizedCursorCoordinate(1.2)).toBe(1);
    expect(normalizedCursorCoordinate(Number.NaN)).toBe(0);
  });
});

describe("PCB cursor coordinates", () => {
  it("scopes peers to the active board", () => {
    expect(pcbCursorSurface("board-1")).toBe("pcb:board-1");
    expect(pcbCursorSurface("board-1")).not.toBe(pcbCursorSurface("board-2"));
  });
});
